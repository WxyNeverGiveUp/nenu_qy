	...
	S.augment(addNewStudent , {
        ...
        _verify: function() {            
            ...
            $(el.J_newStudentSubmit).on("click", function(event){
                ...
                // 姓名
                if(!(/[\u4E00-\u9FA5]{2,5}(?:·[\u4E00-\u9FA5]{2,5})*/.test($(".studentName").val())) && ($(".studentName").val().length!=0)){
                    $(".J_stuName").show();
                    $(".studentName").val(""); 
                };
                // 身份证号
                if(!(/^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/.test($(".idNumber").val())) && ($(".idNumber").val().length!=0)){
                    $(".J_idNumber").show();
                    $(".idNumber").val("");                 
                };   
                // 学号
            ...
            $(".studentName,.idNumber,.studentNumber,.candidateNum").on("click", function(event){
