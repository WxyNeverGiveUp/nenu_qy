...
KISSY.add('detail/detail',function(S){
	var
		...
		el = {
            ...
            J_save_submit2 : '.save-submit2',
			J_pop_check : '.pop-check',
            J_pop_check2 : '.pop-check2',
			J_pop_close : '.pop-close',
			J_edit_result : '.edit-result',
            J_edit_result2 : '.edit-result2',
			J_show : '.show',
			J_check : '.check',
            J_check2 : '.check2',
			J_back : '.back',
		...
		_addEventListener:function(){
				...
				$(el.J_save_submit).on("click",function(ev){
					...
					if(isChooseFit&&isOtherFit){
                        ...
                        // 增添新的判断 如果副书记不通过 导员也变成不通过 并且变更理由
                        if ($(".show tr[id=deputySecretary] > td:nth-child(2)").text() == "待修改") {
                            $(".show tr[id=counsellor] > td:nth-child(2)").text("待修改");//辅导员审核结果也变成待修改
                            $(".show tr[id=counsellor] > td:nth-child(3)").text($(".show tr[id =deputySecretary] > td:nth-child(3)").text());//理由与副书记相同
                        }
						var
            ...
            $(el.J_save_submit2).on("click",function(ev){
                ev.preventDefault();//阻止表单提交按钮的默认行为，验证成功后ajax序列化表单后提交
                //审核理由的验证
                var
                    other_val = $(el.J_check_other_input).val();
                option_innerText = option.text();
                isChooseFit = true;isOtherFit = true;
                //如果审核理由验证成功的话，序列化表单提交
                if(isChooseFit&&isOtherFit){
                    var marked =  $('input[name=checkLevel]' , el.J_check2).val();//取审核表单中隐藏的当前点击行的级别名称，这是判断要把修改后的审核结果写入哪一行的关键
                    var	status="已打回" ;

                    var	reason ="已打回";//操作之后的新理由
                    ...
                    var currentdate = date.getFullYear() + seperator1 + month + seperator1 + strDate
                        ...
                        + seperator2 + strSec;//当前的修改时间
                    var	operator = $(".user-info strong").text();//为当前的登录名，即为操作者
                    $(".show tr[id='school'] > td:nth-child(2)").text(status);//改结果
                    $(".show tr[id='school'] > td:nth-child(3)").text(reason);//改原因
                    $(".show tr[id='school'] > td:nth-child(4)").text(currentdate);//改时间
                    $(".show tr[id='school'] > td:nth-child(5)").text(operator);//改操作者
					// 辅导员与副书记审核结果变成未审核
                    $("#counsellor td:nth-child(2)").text("未审核");
                    $("#counsellor td:nth-child(3)").empty();
                    $("#counsellor td:nth-child(4)").empty();
                    $("#counsellor td:nth-child(5)").empty();
                    $("#deputySecretary td:nth-child(2)").text("未审核");
                    $("#deputySecretary td:nth-child(3)").empty();
                    $("#deputySecretary td:nth-child(4)").empty();
                    $("#deputySecretary td:nth-child(5)").empty();
                    var
                        para = S.io.serialize(".check2");
                    DetailIO.DetailCheck(para,function(code,data,msg){
                        if(code == 0){
                            Dialog.alert(myvar.submitSucTip);
                        }else{
                    ...
                    $(el.J_pop_check2).hide();
                };
            });
            // 防止重复提交
			$(el.J_remark_submit).on('click',function () {
				$(el.J_remark_submit).attr('disabled','disabled');
				document.getElementById('note-form').submit();
				$(el.J_remark_submit).val('已提交');
            });
            //修改结果弹出层显示
                ...
                $('input' ,el.J_edit_result2).on("click",function(ev){
                    $(el.J_pop_check2).show();
                    $('input[name=checkLevel]' , el.J_check2).val($(this).attr("check-level"));//这是在审核表单中隐藏的区域，用来装当前点击行的级别名称
                    //设置当点击修改结果按钮的时候，弹出审核状态都默认都通过，下拉框都默认为请选择
