...
KISSY.add('page/admin_page/students_management/school_roll/alterInfo' , function(S, suggest, alter, linkage, schoolRoll){
    ...
    requires:['alterInfo/suggest', 'alterInfo/alter', 'alterInfo/linkage', 'alterInfo/schoolRoll']
});
/*---------------------------------------------------------------------------*/
/*---------------------------------suggest------------------------------------*/
        ...
        S.augment(alter, {
            ...
            _verify: function() {
                $(el.J_submit).on("click", function(event){
                    ...
                    // 姓名
                    if(!(/[\u4E00-\u9FA5]{2,5}(?:·[\u4E00-\u9FA5]{2,5})*/.test($(".J_name").val())) && ($(".J_name").val().length!=0)){
                        event.preventDefault();
                        $(".J_name_error").show();
                        $(".J_name").val("");
                    };
                    // 身份证号
                    if(!(/^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/.test($(".J_id").val())) && ($(".J_id").val().length!=0)){
                        event.preventDefault();
                        $(".J_id_error").show();
                        $(".J_id").val("");
                    };
