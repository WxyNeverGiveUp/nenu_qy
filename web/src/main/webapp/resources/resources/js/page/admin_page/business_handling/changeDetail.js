    ...
    S.augment(detail , {
        init:function(){
            ...
            var
                ...
                isDateFit = true;//预约时间不为空则为true
                isAddressFit = true;//预约地点不为空则为true
            ...
            // 	};//else
            // };//protocolVerify()
            $(".new-yes input[name=agreementNumber]").on("blur",function(ev){
                var new_protocol = $('input[name=agreementNumber]' , el. J_new_protocol).val(),
                    new_protocol_val_length = new_protocol.length;
                if(!(/^[0-9]{1,20}$/.test(new_protocol)) || (new_protocol == "") || (new_protocol_val_length > 20) ){
                    $(el.J_new_protocol_tip).text(myvar.protocolFalseTip)
                        .removeClass("protocol-tip-ok");
                    isProtocolFit = false;
                }else{
                    isProtocolFit = true;//如果和后台套页的时候有错，把这个注释掉看看
                    //这边是验证新协议编号是否已经存在，如果存在就要给用户一个反馈
                    ...
                    para = 'agreementNumber='+$('input[name=agreementNumber]' , el.J_new_protocol).val();
                     changeDetailIO.changeDetailProtocol(para,function(code,data,msg){
                    	...
                    	if(code == 1){
                            isProtocolFit = true;
                            $(el.J_new_protocol_tip).addClass("protocol-tip-ok");
                    	};
                    });//IO
                //     var para = $(".distribute input").val();
                //     changeDetailIO.changeDetailProtocol({agreementNumber:para},function(rs,data,msg){
                //         if(rs){
                //             var result_query = 0;
                //             S.each(data,function(i,o){
                //                 if(para == i.agreementNumber) {
            ...
            $(el.J_save_submit).on("click",function(ev){
                ...
                var state_input = $('.check-reason select');
                var new_protocol = $('input[name=agreementNumber]' , el. J_new_protocol).val();
                var status_value = $(".check-state input:checked").val();
                if(new_protocol == ""&&status_value == 2) {
                    $(el.J_new_protocol_tip).text(myvar.protocolFalseTip)
                                            .removeClass("protocol-tip-ok");
                    isProtocolFit = false;
                }else {
                ...
                var date_val=$(".laydate-icon").val();
                if (date_val == ""&& status_value == 2) {
                    $(".laydate-icon").css("border-color","#F61628");
                    isDateFit = false;
                }else{
                    isDateFit = true;
                };
                var address_val=$(".address option:selected").val();
                if (address_val == "-1" && status_value == 2) {
                    $(".address").css("border-color","#F61628");
                    isAddressFit = false;
                }else{
                    isAddressFit = true;
                };
                if(isProtocolFit&&isChooseFit&&isOtherFit&&isDateFit&&isAddressFit){
                    ...
                    var	status ;
                    var state_input = $('.check-state input');
                    for(var i=0;i<state_input.length;i++){
                        if(state_input[i].checked==true){
                            if (state_input[i].value == 2){
                                ...
                                $(".show tr[id= "+marked+"] > td:nth-child(2)").text("审核通过");//改结果
                    ...
                    changeDetailIO.DetailCheck(para,function(code,data,msg){
                        if(code == 0){
            ...
            $('input' , el.J_edit_result).on("click",function(ev){
                ...
                $('.check-state .pass').attr('checked','true');
                $(el.J_new_protocol).show();
                $(".new-protocol input[name=distributeNew]").removeAttr("disabled");
