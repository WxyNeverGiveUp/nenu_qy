...
KISSY.add('recruitList/search',function(S){
	var
		$ = S.all, DOM = S.DOM, query = DOM.query, on = S.Event.on, delegate = S.Event.delegate,
		...
		el = {
			...
			dateSort:'.date-sort',
			dateSortInput:'.J_datesort-input',
			regularSort:'.regular-sort',
			recruitDate:'.J_recruit-date'
		},
		cssClASS = {
			dateSort:'date-sort',
			regularSort:'regular-sort'
	...
	S.augment(search,{
		...
		_addEventListener:function(){
			var
				//dateSortLock = true,//按日期排序锁
				//regularSortLock = false,//默认排序锁
				that = this;
			...
			$('a', el.recruitDate).on('click',function(evt){
				if($(evt.currentTarget).hasClass(el.regularSort)){
						$(el.dateSortInput).val(0);
						$(evt.currentTarget).removeClass(el.regularSort);
						$(evt.currentTarget).addClass(cssClASS.dateSort);
						var
							info = that._serialize();
						that._paging(info);
				}
				else if($(evt.currentTarget).hasClass(el.dateSort)){
					$(el.dateSortInput).val(1);
					$(evt.currentTarget).removeClass(el.dateSort);
					$(evt.currentTarget).addClass(cssClASS.regularSort);
					var
						info = that._serialize();
					that._paging(info);
				}
