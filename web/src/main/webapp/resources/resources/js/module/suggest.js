		...
		_addEventListener:function(){
			...
			on(el.areaHolder,'input propertychange',function(evt){
				...
				for(var i=0;i<areaInfo.length;i++){
					...
					if(val==areaInfo[i].cnName){
						$(target).siblings('.unitTip').hide().text('');
					}else{
						$(target).siblings('.unitTip').show().text('请从列表选择正确的地址');
					}
				}
			});
			...
			delegate(document,'input propertychange',el.unitAreaHolder,function(evt){
				...
				for (var i = 0; i < li.length; i++) {
					if(li[i].innerHTML == val){
						...
						$('#J_unitAreaHolder').siblings('.unitTip').text('');
					}
					else{
						$('.J_unit_code').val('');
					}
			...
			on(el.unitAreaHolder,'blur',function(evt){
				var
					target = evt.target,
					val = $(target).val();
				if($(target).siblings('.unitTip').text()!=''){
					$(el.unit_code).val('');
				}
			});
			/*点击其他地方*/
			...
			delegate(document,'input propertychange',el.localAreaHolder,function(evt){
                ...
                for (var i = 0; i < li.length; i++) {
                    if(li[i].innerHTML == val){
                        $('.J_local_code').val(li[i].getAttribute('areacode'));
                        $('#J_localAreaHolder').siblings('.unitTip').text('');
                    }
					else{
						$('.J_local_code').val('');
					}
			...
			on(el.localAreaHolder,'blur',function(evt){
				var
					target = evt.target,
					val = $(target).val();
				if($(target).siblings('.unitTip').text()!=''){
